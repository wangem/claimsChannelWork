...
public class XmlFilter extends BaseFilter {
    ...
    public boolean shouldFilter() { 
		 ...
		 if(proxy.equals(ConstantUtile.ZuulRoutesPath.XML_TO_XML.toString())) {
			 return true;
			} 
		return false; 
    }
...
//        logger.info("{} request to {}", request.getMethod(), request.getRequestURL().toString());
        ...
        logger.info("XmlFilter fileType {}, request from {},queryString=={}",this.getFilterType(), url,queryString);
      //打印头信息
        Enumeration<String> names = request.getHeaderNames();
        
        logger.info("=======XmlFilter==Print head information start =================================="); 
        while(names.hasMoreElements()){
          String name = (String) names.nextElement();
          logger.info(name + ":" + request.getHeader(name));
              
        }
        logger.info("=======XmlFilter===Print head information end ==================================");
